...
Intax = add!(m, Parameter(:Outtax, indices=(goods,), value=0.))
...
for i in goods
    @production(m, Y[i], 0, 1, [Output(PC[i], supply[i], [MPSGE.Tax(:(1 * $(Outtax[i])), RA)])], [Input(PF[:l], factor[i,:l], [MPSGE.Tax(:(1 * $(Intax[i])), RA)]), Input(PF[:k], factor[i,:k])])
end
