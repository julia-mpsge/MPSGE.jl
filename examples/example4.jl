...
c0 = DenseAxisArray(Floate64[2, 4], goods)
e0 = DenseAxisArray(Floate64[sum(fd0[f,:]) for f in factors], factors)
...
for j in sectors
    @production(m, X[j], 1, [Output(PC[i], make0[i,j]) for i in goods], [[Input(P[i], use0[i,j]) for i in goods]; [Input(PF[f], fd0[f,j]) for f in factors]])
end
