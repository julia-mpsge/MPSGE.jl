...
!!! note
      ...
      * After being defined, as below, to access model Parameters in model functions, they must be part of expressions (like a sentence of code), so for referencing the value of `sub_elas_x`, it must be within an evaluated expression, like  
      * ```
        :(1 * $sub_elas_x)
        ```
        rather than julst `sub_elas_x` 
       ...
       - Extra spaces aren't a problem in Julia, so can we align parallel elements - for instance all the transformation elasticities, the substitution elasticities, OUtputs, and Inputs in our 3 production functions - to help readability.
### Add Parameters
...
!!! note
    ...
    * Production Functions don't need names assigned (that is we don't have `X = add!()`) bc they are named by the sector.
```julia
