@testitem "Production - Repeat Indices" begin
    ...
    msg = r"At.*: `@nest\(err\[i = I\].*"
    @test_throws msg @macroexpand(@production(M, X[i=I], [s=0, t=1, err[i=I] => s = 0], begin
            @input(PX[j=I], 100, s)
            @output(PX[j=I], 200, t)
        end))
    ...
    msg = r".*The netput PX\[a\] has index variables Any\[:i\] that are already used in the sector\..*"
    @test_throws msg  @production(M, X[i=I], [s=0, t=1, err[j=I] => s = 0], begin
            @input(PX[i=I], 100, s)
            @output(PX[i=I], 200, t)
        end) 
    ...
    msg = r".*The index i appears more than once. The index associated with each set must be unique.*"
    @test_throws msg @macroexpand(@production(M, Y[i=I,i=I], [s=0, t=1, err[j=I] => s = 0], begin
            @input(PX[i=I], 100, s)
            @output(PX[i=I], 200, t)
        end))
