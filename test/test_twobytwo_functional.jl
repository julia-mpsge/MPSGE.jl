@testitem "TWOBYTWO (functional version)" begin
    ...
    @test JuMP.value(m._jump_model[:Y]) ≈ two_by_two_scalar_results["Y.L","benchmark"]#    1.0
    ...
    @test JuMP.value(m._jump_model[:PX]) ≈ two_by_two_scalar_results["PX.L","benchmark"]#    1.0
    @test JuMP.value(m._jump_model[:PY]) ≈ two_by_two_scalar_results["PY.L","benchmark"]#    1.0
