...
parameter   endow     index of labour endowment     / 1.0 /
       ...
       Otax  tax on output x     /0/
       Itax  tax on input L for x /0.2/;

*       MPSGE model declaration follows

$ontext
$MODEL:twobytwo_wPrice
...
$PROD:X s:esub_x !t:0
        O:PX   Q:110   A:RA  T:Otax  
        I:PL   Q:50    P:1.2  A:RA  T:Itax  ! Variable LX in the algebraic model
...
$PROD:U s:esub_u !t:0
        O:PU   Q:164   !A:RA T:-Itax
        I:PX   Q:110   !P:1.2  A:RA T:-Itax! Variable DX in the algebraic model
...
$sysinclude mpsgeset twobytwo_wPrice
PX.LO = 0.001; PY.LO = 0.001; PU.LO = 0.001; PL.LO = 0.001; PK.LO = 0.001;

* Benchmark replication
PU.FX= 1;
twobytwo_wPrice.iterlim = 0;

$include twobytwo_wPrice.GEN
solve twobytwo_wPrice using mcp;
abort$(abs(twobytwo_wPrice.objval) gt 1e-7) "*** twobytwo does not calibrate ! ***";
twobytwo_wPrice.iterlim = 1000;
...
execute_unload "twobytwo_wPrice.gdx" equilibrium

*=== Write to variable levels to Excel file from GDX 
*=== If we do not specify a sheet, data is placed in first sheet
execute 'gdxxrw.exe twobytwo_wPrice.gdx o=MPSGEresults.xlsx par=equilibrium rng=two_by_two_PriceinIntput!'
execute 'gdxxrw.exe twobytwo_wPrice.gdx o=C:\Users\Eli\.julia\dev\MPSGE\test\MPSGEresults.xlsx par=equilibrium rng=two_by_two_PriceinIntput!'