**stitle Dataset for a 123Nested Model
...
$title  Static 123Nested Model Ala Devarjan
...
$report:
    v:W     w:HH
    v:PDHHC  d:PD     demand:HH
    v:PMHHC  d:PM     demand:HH
    v:LD     d:PL    demand:HH

$offtext
$sysinclude mpsgeset mge123Nested
...
*abort$(mge123Nested.objval > 1e-4) "Scenario fails to solve.";
...
report("PDHHC","%replacement%") =PDHHC.L;
report("PMHHC","%replacement%") =PMHHC.L;
...
mge123Nested.iterlim = 0;
$include MGE123Nested.GEN
solve mge123Nested using mcp;
*abort$(mge123Nested.objval > 1e-4) "Benchmark model does not calibrate.";

$set replacement Benchmark
$set labormarket N
$include %gams.scrdir%report

mge123Nested.iterlim = 10000;

*mge123Nested.savepoint = 2;
...
execute_unload "123Nested.gdx" report
*=== Write to variable levels to Excel file from GDX 
*=== If we do not specify a sheet, data is placed in first sheet
execute 'gdxxrw.exe 123Nested.gdx o=MPSGEresults.xlsx par=report rng=The123Nested!'
execute 'gdxxrw.exe 123Nested.gdx o=C:\Users\Eli\.julia\dev\MPSGE\test\MPSGEresults.xlsx par=report rng=The123Nested!'
*